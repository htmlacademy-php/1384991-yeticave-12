<main>
    ...
    --invalid -->
      ...
      <div class="form__item <?php if($err['email']): ?> form__item--invalid<?php endif; ?>"> <!-- form__item--invalid -->
        ...
        <input id="email" type="text" name="email" placeholder="Введите e-mail" value="<?=clear_spec(getPostVal('email')); ?>">
        <span class="form__error"><?=clear_spec($err['email']); ?></span>
      </div>
      <div class="form__item <?php if($err['password']): ?> form__item--invalid<?php endif; ?>">
        <label for="password">Пароль <sup>*</sup></label>
        <input id="password" type="password" name="password" placeholder="Введите пароль">
        <span class="form__error"><?=clear_spec($err['password']); ?></span>
      </div>
      <div class="form__item <?php if($err['name']): ?> form__item--invalid<?php endif; ?>">
        <label for="name">Имя <sup>*</sup></label>
        <input id="name" type="text" name="name" placeholder="Введите имя" value="<?=clear_spec(getPostVal('name')); ?>">
        <span class="form__error"><?=clear_spec($err['name']); ?></span>
      </div>
      <div class="form__item <?php if($err['message']): ?> form__item--invalid<?php endif; ?>">
        <label for="message">Контактные данные <sup>*</sup></label>
        <textarea id="message" name="message" placeholder="Напишите как с вами связаться" value="<?=clear_spec(getPostVal('message')); ?>"></textarea>
